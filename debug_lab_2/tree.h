...
public:
  ...
  std::tuple<int, node_t *, node_t *> split_leaf() {
    node_t *n_left = new node_t(up, true, left, this); this -> left = n_left; if(n_left -> left){n_left -> left -> right = n_left;} //2
    int mid = key_list.size() / 2;

    n_left->key_list = std::vector<int>(key_list.begin(), key_list.begin() + mid);
    n_left->value_list =
